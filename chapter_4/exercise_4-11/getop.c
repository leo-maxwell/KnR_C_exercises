...
int getop(char s[]) {
    ...
    // handle commands and variables
        ...
        cmd[j++] = c;
        int exp_c = c_buf == EOF ? getch() : c_buf;
        c_buf = EOF;
        if (!isalpha(exp_c)) {
        ...
        // it is interpreted as command as we only prepare for single letter variables at this stage
            ...
            cmd[j++] = exp_c;
            while (isalpha(c = c_buf == EOF ? getch() : c_buf)) {
                c_buf = EOF;
    ...
    if (isdigit(c))
        for (; isdigit(s[++i] = c = c_buf == EOF ? getch() : c_buf); c_buf = EOF); // s[0] is already the first digit in this case, hence s[++i] used
    if (c == '.')
        for (; isdigit(s[++i] = c = c_buf == EOF ? getch() : c_buf); c_buf = EOF); // s[i] is the decimal point, hence s[++i] used
